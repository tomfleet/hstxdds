...
static uint8_t header_idx = 0; // Track cumulative progress
...
// Tracker for cumulative progress
...
void dds_update_config(dds_config_t *config) {
    ...
    SEGGER_RTT_WriteString(0, "DDS: Applied.\n");
}

void process_mailbox() {
    uint32_t now = to_ms_since_boot(get_absolute_time());

    // Safety: If stuck in a state for > 500ms without new data, reset to IDLE
    if (current_state != IDLE && (now - last_byte_time_ms > 500)) {
        current_state = IDLE;
        header_pos = 0;
        data_pos = 0;
        SEGGER_RTT_WriteString(0, "Mbox Timeout: Resetting to IDLE\n");
    }
    ...
    switch (current_state) {
        case IDLE:
            if (SEGGER_RTT_HasData(0)) {
                ...
                if (c == 0xEF) { // Start of 0xDEADBEEF (Little Endian)
        ...
        case READ_HEADER:
            // Grab whatever bytes are currently available, one by one
        ...
        case READ_DATA:
            // Drain Channel 1 as data arrives
...
void process_mailbox2() {

    uint32_t now = to_ms_since_boot(get_absolute_time());

    // Reset to IDLE if we are stuck in a state for more than 500ms without data
    if (current_state != IDLE && (now - last_byte_time_ms > 500)) {
        current_state = IDLE;
        header_pos = 0;
        data_pos = 0;
        SEGGER_RTT_WriteString(0, "Transfer Timeout: Resetting to IDLE\n");
    }
    switch (current_state) {
        case IDLE:
            if (SEGGER_RTT_HasData(0)) {
                uint8_t c;
                //SEGGER_RTT_Read(0, &c, 1);
                //last_byte_time_ms = to_ms_since_boot(get_absolute_time());
                SEGGER_RTT_Read(0, &c, 1);
                if (c == 0xEF) { // Start of 0xDEADBEEF in Little Endian
                    header_work_buf[0] = 0xEF;
                    header_pos = 1;
                    last_byte_time_ms = now;
                    current_state = READ_HEADER;
                 ...
                 else if (c == 'X') { // Manual Reset Character
                    header_pos = 0;
                    data_pos = 0;
                    SEGGER_RTT_WriteString(0, "Forced Reset to IDLE\n");
                }
        ...
        case READ_HEADER:
            // Pull whatever is available, one by one or in chunks
        while (SEGGER_RTT_HasData(0) && header_pos < 20) {
                SEGGER_RTT_Read(0, &header_work_buf[header_pos++], 1);
                last_byte_time_ms = to_ms_since_boot(get_absolute_time());
            }
            ...
            case READ_DATA:
            // Pull data from Channel 1 as it becomes available
            while (SEGGER_RTT_HasData(1) && data_pos < incoming_header.len) {
                uint32_t avail = SEGGER_RTT_GetBytesInBuffer(1);
                uint32_t remaining = incoming_header.len - data_pos;
                uint32_t to_read = (avail < remaining) ? avail : remaining;
            ...
            if (data_pos >= incoming_header.len) {
                ...
                SEGGER_RTT_WriteString(0, "DDS Load Success.\n");
                current_state = IDLE;
                header_pos = 0;
                data_pos = 0;
            }
